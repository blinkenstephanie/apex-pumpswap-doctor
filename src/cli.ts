...
async function main(): Promise<number> {
  ...
  try {
    ...
    for (const p of positional) assertNoSecrets(p);
    for (const v of flags.values()) {
      if (typeof v === "string") assertNoSecrets(v);
    }
  } catch (e) {
  ...
  try {
    if (cmd === "tx") {
      ...
      if (!signature) throw new Error("tx requires <public-signature>");
      diagnosis = await diagnoseTx({
        signature,
        logsFile: flags.get("logs") as string | undefined,
        rpcUrl: flags.get("rpc") as string | undefined,
      });
    } else if (cmd === "repo") {
      diagnosis = diagnoseRepo(positional[1] || ".");
    } else if (cmd === "pool") {
      const mint = positional[1];
      if (!mint) throw new Error("pool requires <mint>");
