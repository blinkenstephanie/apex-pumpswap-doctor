...
export function looksLikeSecret(input) {
...
export function assertNoSecrets(...inputs) {
    for (const input of inputs) {
        if (input == null || input === "")
            continue;
        const hit = looksLikeSecret(input);
        if (hit) {
            throw new Error(`Refused: input looks like a secret (${hit}). ` +
...
export function looksLikePublicBase58(s, min = 32, max = 88) {
